...
; DrawImage macro
; imageTable - binary file
;     first byte represents width of image
;     second byte represents height of image
;     subsequent bytes are evaluated from tope left to bottom right and represent offset within pattern table from bellow starting point offset
; col - where to start from the left -> right, the x coordinate
; row - where to start from the top -> bottom, the y coordinate
; offset - where to start referencing data from the pattern table 2?
...
@loop:
  lda (textPtr), y
  beq @break
  cmp #$0A ; is character a new line character
  beq @new_line ; if so, execute newline routine
  sta PPU_DATA
  jmp @next
@new_line:
  lda vramPtr
...
; DrawText macro
; col - x coordinate from left of screen
; row - y coordinate from top of screen
; label - byte array representing blocks in pattern table 1
; nametable - draw to foreground or background?
