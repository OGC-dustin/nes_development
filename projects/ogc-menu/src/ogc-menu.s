; copy of nes-pi.s - An NES game that computes the digits of pi.
; By NesHacker
; addapted to OGC engineering menu, will erase pi calc section
...
; enumeration of GameState defines the states available in a state machine style control of the system
.enum GameState
  pretitle      = 0 ; displays a company logo
  title         = 1 ; start menu logo and controller input detection
  digit_select  = 2 ; dialog screen in this example to set total digits to calculate
  calculate     = 3 ; main operation loop calculating pi
.endenum
...
.segment "CHARS"
; CHR-ROM.bin loads both pattern tables for use in logo, start menu, etc.
.incbin "./src/bin/ASCII_TABLE.chr"
